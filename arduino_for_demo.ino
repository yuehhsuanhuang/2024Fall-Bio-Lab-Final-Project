...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (receivedData[0] == 'd') {
      ...
      waitBuzzSec = (asleepTime.substring(3).toInt()+sleepDuration.substring(3).toInt())*1000;
  ...
  if (buzzTime != "" && (millis() - startMillis >= waitBuzzSec)) {
    tone(buzzerPin, 1000);
    delay(500);
    noTone(buzzerPin);
  }
...
/*
  ...
  char currentTime[6]; // Buffer to hold the time string in the format hh:mm
